...
| -------------------------------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------- |
| `dependency-type`                | No       | Type of dependencies to include: production \| all                                                                                   | `all`                                 |
| `start-paths`                    | No       | A comma-separated list of paths to scan for licenses. Results are merged into a single report.                                       | `./`                                  |
  ...
  generate-credits:
    ...
    steps:
      ...
      - name: Generating ./app credits
        ...
        with:
          start-paths: './app' # Path to scan for licenses from "repo-name/app" folder
...
## Example usage with workspaces

For monorepos using workspaces (npm, pnpm, or yarn), pointing `start-paths` at the repo root will automatically discover all workspaces and produce a combined license report:

```yaml
- name: Generate combined credits file for all packages
  uses: dequelabs/internal-actions-public/.github/actions/npm-license-checker@main
  with:
    start-paths: './'
    dependency-type: 'production'
    details-output-path: './credits.json'
    details-output-format: 'json'
```

If you only need a license report for specific workspace packages, list them explicitly:

```yaml
- name: Generate scoped credits file for specific packages
  uses: dequelabs/internal-actions-public/.github/actions/npm-license-checker@main
  with:
    start-paths: './packages/app, ./packages/client'
    dependency-type: 'production'
    details-output-path: './credits.json'
    details-output-format: 'json'
```
